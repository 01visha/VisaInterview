  ...
  const login = useGoogleLogin({    
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        //console.log("Login API Response:", loginResponseData); // Log login API response
  
        // If login is successful (200 or 201), redirect to dashboard
        if (loginResponse.ok) {        
          handleSuccessfulLogin(userData, loginResponseData);
          return;
        }
  
        // If login fails with 401 and message "User not found", register the user
        if (
          loginResponse.status === 401 &&
          loginResponseData.message === "User not found"
        ) {
          ...
          const registerPayload = {
            ...
            password: userData.sub, // Use Google user_id as password
            full_name: userData.name || "", // Use Google name if available
            sso: 1, // Indicates Google SSO
            subscription_id: subscriptionId, // Example subscription ID
            trial_subscription_id: trialsubscriptionId, // Example trial subscription ID
            order_type: ordertype, // Example order type
          };
          ...
          if (!registerResponse.ok) {
            throw new Error(
              registerResponseData.message || "Registration failed"
            );
          }
  
          handleSuccessfulLogin(userData, registerResponseData);
  
          // Store the auth_code in local storage
          localStorage.setItem("auth_code", registerResponseData.token);
  
          // Store Google user data in localStorage for profile update
          localStorage.setItem("googleUserData", JSON.stringify(userData));

          // Redirect to /profile-update for additional fields
          navigate("/profile-update");

        } else {
          // Handle other login errors
          throw new Error(loginResponseData.message || "Login failed");
        }
      } catch (error) {
        console.error("Error during login process:", error); // Log general error
        toast.error(
